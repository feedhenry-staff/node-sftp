...
(function() {
    ...
    this.connect = function(cbconnect) {
        ...
        this.socket.on("data", function(data) {
            ...
            parseReply.call(_self, data.toString(), 1,function(err){
                if (err)
                    return cbconnect(err,null);
            });
        });
    ...
    this.streamFile = function(filename, encoding, callback) {
        var temp = Util.DEFAULT_TMPDIR + "/streamFile";

        this.exec("readFile", "get " + filename + " " + temp, function(lines) {
            // 'Couldn\'t stat remote file: No such file or directory', ERROR HANDLING
            var err = isError(lines);
     ...
     * @private
     */
    function parseReply(data, string, callback) {
        // console.log("data: ", data, data.split("\n").length + " parts");
        if (data.indexOf("Permission denied") > -1) {
            return callback(data,null);
        }
        if (data.indexOf("Connection closed by remote host") > -1) {
            return callback(data,null);
        }
        this.emit("data", data);
